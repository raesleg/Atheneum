...
$extraJS = [["src" => "assets/js/contact.js?v=2", "defer" => true]];
...
if ($isLoggedIn && $userId) {
    try {
        $stmt = $conn->prepare("
            SELECT o.orderId, o.totalPrice, o.created_at, u.fname, u.lname, u.email
            FROM Orders o
        ...
        $stmt->execute();
        $res = $stmt->get_result();
        $eligibleOrders = $res->fetch_all(MYSQLI_ASSOC);
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== ($_SESSION['csrf_token'] ?? '')) {
        ...
        if ($subject === 'Refund') {
            ...
            if (!$isLoggedIn) {
                ...
                try {
                    $stmt = $conn->prepare("INSERT INTO Refund (orderId, userId, reason, status, type, name, email, subject) VALUES (?, ?, ?, 'pending', 'Refund', ?, ?, 'Refund')");
                    $stmt->bind_param("iisss", $orderId, $userId, $message, $name, $email);
                    if ($stmt->execute()) {
                        $success = true;
                        $values = [];
                    } else {
            ...
            if (empty($errors)) {
                ...
                // Insert into Refund table whether they are guests or users
                $stmt = $conn->prepare("
                    INSERT INTO Refund (userId, type, name, email, subject, reason, status)
                    VALUES (?, ?, ?, ?, ?, ?, 'pending')
                ");

                // userId can be NULL if guest
                $insertUserId = $isLoggedIn ? $userId : null;
                $stmt->bind_param("isssss", $insertUserId, $db_type, $name, $email, $db_subject, $message);

                if ($stmt->execute()) {
                    $success = true;
                    $values = [];
                } else {
                    $errors[] = 'Could not submit your message. Please try again later.';
                }
                $stmt->close();
            }
        }
...
if ($isLoggedIn && $userId) {
    $stmt_user = $conn->prepare("SELECT fname, lname, email FROM Users WHERE userId = ?");
    $stmt_user->bind_param("i", $userId);
    $stmt_user->execute();
    $userRow = $stmt_user->get_result()->fetch_assoc();
    $stmt_user->close();

    if ($userRow && ($_SERVER['REQUEST_METHOD'] !== 'POST' || $success)) {
        $values['name'] = trim(($userRow['fname'] ?? '') . ' ' . $userRow['lname']);
        $values['email'] = $userRow['email'];
    }
}
...
<main>
    ...
    <div class="container-fluid px-4 px-lg-5 contact-layout">
        ...
        <aside class="contact-info" aria-label="Contact information">
            ...
            <div class="contact-map">
                <iframe
                    ...
                    title="Map showing Atheneum at 1 Raffles Place, Singapore" loading="lazy" aria-hidden="true"
                    tabindex="-1">
                </iframe>
            </div>
        </aside>
        ...
        <div class="contact-form-col">
            ...
            <form method="POST" action="contact.php" class="contact-form" novalidate aria-label="Contact form">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="contactName" name="name" class="form-input-custom"
                            value="<?= htmlspecialchars($values['name'] ?? '') ?>" placeholder="Jane Doe"
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" id="contactEmail" name="email" class="form-input-custom"
                            value="<?= htmlspecialchars($values['email'] ?? '') ?>" placeholder="jane@example.com"
                            autocomplete="email" required>
                    </div>
                </div>

                <!-- Subject is a select dropdown -->
                <div class="form-group">
                    <label for="contactSubject" class="form-label-custom">How can we help? <span
                            class="req">*</span></label>
                    <select id="contactSubject" name="subject" class="form-input-custom" required>
                        <option value="">-- Choose a subject --</option>
                        <option value="General Enquiry" <?= ($values['subject'] ?? '') === 'General Enquiry' ? 'selected' : '' ?>>General
                            Enquiry</option>
                        <option value="Refund" <?= ($values['subject'] ?? '') === 'Refund' ? 'selected' : '' ?>>Request a
                            Refund</option>
                        <option value="Feedback" <?= ($values['subject'] ?? '') === 'Feedback' ? 'selected' : '' ?>>
                            Feedback</option>
                    </select>
                </div>
                        ...
                        <select name="orderId" id="orderId" class="form-input-custom">
                            ...
                            <?php foreach ($eligibleOrders as $order): ?>
                                <option value="<?= $order['orderId'] ?>">
                                    Order #<?= $order['orderId'] ?> ($<?= number_format($order['totalPrice'], 2) ?>) -
                ...
                <div class="form-group">
                    ...
                    <textarea id="contactMessage" name="message" class="form-input-custom" rows="7"
