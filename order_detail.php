...
<main>
    <div class="order-detail-wrapper">
        <nav aria-label="Breadcrumb">
            <a href="orders.php" class="back-link"><i class="bi bi-arrow-left" aria-hidden="true"></i> Back to Orders</a>
        </nav>
                ...
                <?php foreach ($items as $item): ?>
                    <div class="order-item-row" role="listitem" aria-label="<?= htmlspecialchars($item['title']) ?>, quantity <?= $item['quantity'] ?>">
                        <?php if (!empty($item['cover_image'])): ?>
                            <img src="<?= htmlspecialchars(str_replace(' ', '%20', $item['cover_image'])) ?>"
                        ...
                        <div class="order-item-right">
                            ...
                            <?php if (isset($reviewEligible[$item['productId']])): ?>
                                <a href="book.php?id=<?= $item['productId'] ?>#reviewFormCard" class="write-review-link" aria-label="Write a review for <?= htmlspecialchars($item['title']) ?>">
